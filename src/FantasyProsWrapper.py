...
__all__ = ['FantasyPros', 'NHL', 'NFL', 'Draft Aid', 'Weekly Lineups']
__version__ = '1.1'
__author__ = 'joelwhitney'
__email__ = 'whitney.joel.b@gmail.com'
'''
...
"""
setup the logger for the module to print to the console and log to file
"""
formatter = logging.Formatter("[%(asctime)s] [%(filename)30s:%(lineno)4s - %(funcName)30s()]\
     [%(threadName)5s] [%(name)10.10s] [%(levelname)8s] %(message)s")  # The format for the logs
logger = logging.getLogger(__name__)  # Grab the root logger
logger.setLevel(logging.DEBUG)  # Set the root logger logging level (DEBUG, INFO, WARNING, ERROR, CRITICAL)
sh = logging.StreamHandler(sys.stdout)
sh.setFormatter(formatter)
sh.setLevel(logging.DEBUG)
rh = logging.handlers.RotatingFileHandler('FantasyProsWrapperLog.txt', mode='a', maxBytes=10485760)
...
class NHL(object):
    """
    NHL object for getting week, start date, etc.
    """
...
class FantasyProsFootball(object):
    ...
    def playerids_from_html(self, full_file_name):
        """
...
class FantasyProsPlayerResults(object):
...
class FantasyProsPlayer(object):
    pass
